...
# A $1 increase to a country's GDP per capita can be associated with a .00076 year
...
# The coefficient on `gdpPercap` is not statistically significant at the 1% level.
...
# This model explains 34% of the variation in lifeExp.
...
# When GDP per capita increases by 1%, life expectancy will increase by 14.7%.
...
lm(formula = lifeExp ~ I(gdpPercap ^2), data = gapminder) %>%
  broom::glance()

# This model explains 4.89% of the variation in lifeExp.
...
lm(formula = lifeExp ~ 0 + gdpPercap, data = gapminder) %>%
  broom::tidy(conf.int = TRUE)
...
# life expectancy of 47.9+8.66=56.56 years.
